...
onclick = () => {
	if (demoInter == 0) {
		...
		setTimeout(() => {
			clearInterval(demoInter); demoInter = setInterval(() => { pressDown(text[index]) }, 43);
		}, 11900)

		setTimeout(() => {
			clearInterval(demoInter); demoInter = setInterval(() => { pressDown(text[index]) }, 42.19);
		}, 23680)

		setTimeout(() => {
			clearInterval(demoInter); demoInter = setInterval(() => { pressDown(text[index]) }, 36);
		}, 36000)
		...
		setTimeout(() => {
			clearInterval(demoInter); demoInter = setInterval(() => { pressDown(text[index]) }, 26.1);
		}, 72000)

		setTimeout(() => {
			clearInterval(demoInter); demoInter = setInterval(() => { pressDown(text[index]) }, 90);
		}, 101700)
...
setInterval(() => {
	...
	listTimeOfValidClicks.forEach((value) => {
		if (Date.now() - 1000 > value) delite = true;
	})
	if (delite) listTimeOfValidClicks.shift();
	strelka.style.transform = "translate(214px, 20px) rotate(" + (-94.5 + 0.23625 * listTimeOfValidClicks.length * 60) + "deg)"
	speedValue.innerText = listTimeOfValidClicks.length * 60
})
